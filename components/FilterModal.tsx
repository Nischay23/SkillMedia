...
const ProgressBar = ({ currentLevel, maxLevels }: { currentLevel: number; maxLevels: number }) => {
  const { theme, isDark } = useTheme();
  const progress = useSharedValue(0);

  useEffect(() => {
    progress.value = withTiming((currentLevel + 1) / maxLevels, {
      duration: 300,
...
export default function FilterModal({
  ...
  // In-memory caches
  const childrenByParentRef = useRef<Map<string | undefined, FilterOption[]>>(new Map());
  ...
  useEffect(() => {
    if (children) {
      ...
      childrenByParentRef.current.set(parentId as unknown as string | undefined, list);
  ...
  useEffect(() => {
    if (breadcrumbOptions) {
      ...
      for (const opt of breadcrumbOptions) {
        if (opt) {
          optionByIdRef.current.set(opt._id as unknown as string, opt as unknown as FilterOption);
        }
      }
    }
  }, [breadcrumbOptions]);
  ...
  const currentTitle = LEVEL_TITLES[Math.min(path.length, LEVEL_TITLES.length - 1)];
  ...
  const childrenToShow = (children ??
    childrenByParentRef.current.get(parentId as unknown as string | undefined) ??
  ...
  const renderGrid = () => {
    if (children !== undefined && childrenToShow.length === 0) {
      return (
    ...
    >
      ...
      <View style={styles.backdrop}>
        ...
        >
          ...
          {/* Progress Bar */}
          <ProgressBar currentLevel={path.length} maxLevels={MAX_FILTER_LEVELS} />
