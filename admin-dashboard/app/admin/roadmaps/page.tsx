...
export default function RoadmapsPage() {
  ...
  const [deleteRoadmapId, setDeleteRoadmapId] = useState<Id<"roadmaps"> | null>(null);
  ...
  const filteredRoadmaps = roadmaps?.filter((roadmap) => {
    // Status filter
    if (statusFilter === "published" && !roadmap.isPublished) return false;
    if (statusFilter === "draft" && roadmap.isPublished) return false;
  ...
  const stats = {
    ...
    published: roadmaps?.filter((r) => r.isPublished).length ?? 0,
    draft: roadmaps?.filter((r) => !r.isPublished).length ?? 0,
    totalUsers: roadmaps?.reduce((sum, r) => sum + (r.usersStarted ?? 0), 0) ?? 0,
  };
  ...
  const handleTogglePublish = async (roadmapId: Id<"roadmaps">, currentStatus: boolean) => {
    try {
      await updateRoadmap({ roadmapId, isPublished: !currentStatus });
    } catch (error) {
  ...
  const availableGroups = groups?.filter((g) => !g.hasRoadmap) ?? [];
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Create and manage learning roadmaps for career groups
          </p>
        </div>
        <button
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                filteredRoadmaps.map((roadmap) => (
                  <tr key={roadmap._id} className="transition-colors hover:bg-muted/50">
                    {/* Roadmap */}
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-foreground">{roadmap.title}</p>
                          {roadmap.difficulty && (
                            <span
                    ...
                    {/* Steps */}
                    <td className="px-6 py-4 text-center">
                      <span className="text-sm text-foreground">{roadmap.stepCount}</span>
                    </td>

                    {/* Users */}
                    <td className="px-6 py-4 text-center">
                      <span className="text-sm text-foreground">{roadmap.usersStarted}</span>
                    </td>

                    {/* Status */}
                    <td className="px-6 py-4">
                      <button
                        onClick={() => handleTogglePublish(roadmap._id, roadmap.isPublished)}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => setDeleteRoadmapId(roadmap._id)}
                  ...
                  <td colSpan={8} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-muted-foreground">
                        {searchQuery ? "No roadmaps match your search" : "No roadmaps found"}
                      </p>
                      {!searchQuery && (
                        <button
                          onClick={() => setShowCreateModal(true)}
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-foreground">Create New Roadmap</h2>
            ...
            <div className="mt-6 space-y-4">
              ...
              <div>
                ...
                <select
                  value={newDifficulty}
                  onChange={(e) => setNewDifficulty(e.target.value as typeof newDifficulty)}
                  className="input-field"
                >
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button
                ...
                disabled={!selectedGroupId || !newTitle.trim() || isCreating}
